...
public class InitDb {
	...
	static class InitService {
		...
		public void departmentMemberInit() {
			...
			DailyRentalTime dailyRentalTime = DailyRentalTime.createDailyRentalTime(
				LocalTime.of(10, 0),
				LocalTime.of(17, 55),
			...
			for (DayOfWeek day : DayOfWeek.values()) {
				weeklyRentalTime.put(day, dailyRentalTime);
			}
			Department departmentComputer = Department.createDepartment("컴퓨터공학과", weeklyRentalTime);
			em.persist(departmentComputer);
			Department departmentData = Department.createDepartment("산업데이터공학과", weeklyRentalTime);
